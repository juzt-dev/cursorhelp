...
1. **Search Strategy**:
   - Check if `gemini` bash command is available, if so, execute `gemini -p "...your search prompt..."` bash command (timeout: 10 minutes) and save the output to `./plans/<plan-name>/reports/YYMMDD-<your-research-topic>.md` file (including all citations).
