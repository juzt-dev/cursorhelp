...
- Write a prompt for 'SCALE' number of agents that will immediately call the `Bash` tool to run these commands to kick off your agents to conduct the search:
  - `gemini -p "[prompt]"` (if count <= 6)
  - if count > 6, spawn default codebase search tools to search the codebase in parallel
...
- If `gemini` is not available, ask the user if they want to install it:
  - If **yes**, install it (if there are permission issues, instruct the user to install it manually, including authentication steps)
  - If **no**, use default codebase search tools.
- IMPORTANT: Kick these agents off in parallel, analyze and divide folders for each agent to scout intelligently and quickly.
...
- IMPORTANT: Immediately call the Bash tool to run the respective agentic coding tool (gemini, etc.)
...
- Instruct each agent to use a timeout of 3 minutes for each bash call. Skip any agents that don't return within the timeout, don't restart them.
